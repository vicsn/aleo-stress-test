...
start_snarkos_in_tmux() {
  ...
  ssh -o StrictHostKeyChecking=no aws-n$NODE_ID << EOF
    ...
    tmux send-keys -t "snarkos-session" "snarkos start --nodisplay --bft 0.0.0.0:5000 --rest 0.0.0.0:3033 --peers $NODE_IP:4133 --validators $NODE_IP:5000 --verbosity $VERBOSITY --dev $NODE_ID --dev-num-validators $NUM_INSTANCES --validator --nocdn" C-m
