...
def get_committee(query_node_id):
    # Calculate the port number
    port = 3030 + query_node_id

    # Construct the URL
    url = f"http://127.0.0.1:{port}/testnet3/latest/committee"
...
def get_staked_balances(query_node_id):
    staked_balances = {}
    committee = get_committee(query_node_id)
    for member_address in committee["members"]:
        committee_values = committee["members"][member_address]
        node_id = addresses.index(member_address)
        staked_balances[node_id] = committee_values[0]
    return staked_balances

def unbond_credit(node_id, credit_amount, message_node_id):
    # Calculate the port number for broadcasting the transaction
    port = 3030 + message_node_id

    # Prepare the command
    private_key = private_keys[node_id]
    broadcast_url = f"http://127.0.0.1:{port}/testnet3/transaction/broadcast"
    query_url = f"http://127.0.0.1:{port}"
    command = f"snarkos developer execute credits.aleo unbond_public {credit_amount}u64 --private-key {private_key} --query {query_url} --broadcast {broadcast_url}"

    print(f"Unbonding {credit_amount} credits from node {node_id}")
    # Execute the command
    try:
        result = subprocess.run(command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return result.stdout.decode()  # Return the output of the command
    except subprocess.CalledProcessError as e:
        return f"Command failed: {e.stderr.decode()}"  # Return the stderr output if the command fails
    except Exception as e:
        return str(e)  # Return the error message for other exceptions
