...
read -p "Enter the EC2 instance name to filter by (e.g. Name) (default: net5): " INSTANCE_NAME
INSTANCE_NAME="${INSTANCE_NAME:-net5}"

# Read the PEM file path from the user or use the default in ~/.ssh
read -p "Enter the PEM file path (default: ~/.ssh/kp2.pem): " PEM_FILE
PEM_FILE="${PEM_FILE:-~/.ssh/kp2.pem}"
