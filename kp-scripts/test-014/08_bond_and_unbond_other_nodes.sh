...
# Extract the HostName/IP for aws-n1
node_one_ip=$(awk '$1=="Host" && $2=="aws-n1"{flag=1;next} $1=="HostName" && flag{print $2;flag=0}' ~/.ssh/config)
echo "IP for aws-n1: $node_one_ip"

for i in {0..4}
do
    echo "unbonding node ${i}"
    snarkos developer execute credits.aleo unbond_public 186300983488812u64 --private-key ${private_keys[i]} --query http://${node_one_ip}:3033 --broadcast http://${node_one_ip}:3033/testnet3/transaction/broadcast
done
...
do
    ...
    do
        echo "bonding node ${i}"
        snarkos developer execute credits.aleo bond_public ${addresses[i]} 1200000000000u64 --private-key ${private_keys[i]} --query http://${node_one_ip}:3033 --broadcast http://${node_one_ip}:3033/testnet3/transaction/broadcast
    done
    for i in {4..19}
    do
        echo "unbonding node ${i}"
        snarkos developer execute credits.aleo unbond_public 1200000000000u64 --private-key ${private_keys[i]} --query http://${node_one_ip}:3033 --broadcast http://${node_one_ip}:3033/testnet3/transaction/broadcast
    done
done