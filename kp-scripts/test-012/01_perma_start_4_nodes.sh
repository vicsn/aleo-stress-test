...
for validator_index in "${validator_indices[@]}"; do
  ...
  tmux send-keys -t "devnet:window$validator_index" "snarkos start --nodisplay --dev $validator_index --dev-num-validators $total_validators --validator --verbosity 0 --logfile $log_file" C-m
done
