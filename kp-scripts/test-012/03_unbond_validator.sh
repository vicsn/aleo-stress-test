...
read -p "Enter the validator index to unbond (default: 3): " val_index
val_index=${val_index:-3}

curl http://127.0.0.1:3030/testnet3/committee/latest

# echo private key of validator to unbond
echo ${private_keys[val_index]}

snarkos developer execute credits.aleo unbond_public "187500000000000u64" --private-key "${private_keys[val_index]}" --query "http://localhost:3030" --broadcast "http://localhost:3030/testnet3/transaction/broadcast"
