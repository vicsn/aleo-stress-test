...
read -p "Enter the total number of validators (default: 16): " total_validators
total_validators=${total_validators:-16}
...
validator_indices=()
for ((i = 2; i < 2+total_validators; i++)); do
    validator_indices+=($i)
done
...
for validator_index in "${validator_indices[@]}"; do
  ...
  if [ "$validator_index" -eq "$last_index" ]; then
    peer_address="127.0.0.1:4132"
    validators_address="127.0.0.1:5002"
  else
