...
for validator_index in "${validator_indices[@]}"; do
  ...
  # Default case: Connect nodes within their respective networks
  if [ "$validator_index" -lt 9 ]; then
    # For the first 9 nodes, wrap around to the first node after the eighth
    next_validator_index=$(( (validator_index + 1) % 9 ))
  else
    # For the last 4 nodes, wrap around to the ninth node after the last node
    next_validator_index=$(( 9 + (validator_index - 9 + 1) % 4 ))
  fi
  peer_port=$((5000 + next_validator_index))
  peer_address="127.0.0.1:$peer_port"
  validators_address="127.0.0.1:$peer_port"

  # Special case adjustments are no longer needed
