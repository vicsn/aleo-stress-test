...
do
    ...
    do
        echo "${i}"
        snarkos developer execute credits.aleo bond_public ${addresses[i]} 1200000000000u64 --private-key ${private_keys[i]} --query "http://3.137.187.113:3033" --broadcast "http://3.137.187.113:3033/testnet3/transaction/broadcast"
    done
    for i in {4..23}
    do
        echo "${i}"
        snarkos developer execute credits.aleo unbond_public 1200000000000u64 --private-key ${private_keys[i]} --query "http://3.137.187.113:3033" --broadcast "http://3.137.187.113:3033/testnet3/transaction/broadcast"
    done
done
...
curl http://3.137.187.113:3033/testnet3/committee/latest
curl http://3.137.187.113:3033/testnet3/program/credits.aleo/mapping/account/aleo1p9sg8gapg22p3j42tang7c8kqzp4lhe6mg77gx32yys2a5y7pq9sxh6wrd