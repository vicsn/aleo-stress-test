...
# Let the user input the number of nodes (default: 8)
number_of_nodes = 8
...
program_deployments_path = os.path.join(os.getcwd(), "transfers")
if not os.path.exists(program_deployments_path):
    os.makedirs(program_deployments_path)

deployment_paths = []
for i in range(start_index, start_index + number_of_program_deployments):
    ...
    cmd = f"tx-cannon batch-transfer --address-list addresses.txt -k {private_key} --amount 1000 -e http://127.0.0.1:3031 -s transfers"
    print("Executing", cmd)
    os.system(cmd)
    deployment_txt_path = os.path.join(program_deployments_path, "transfers.txt")
    # rename the deployment file to numbers similar to the program file
    deployment_txt_path_new = os.path.join(program_deployments_path, f"transfer-{i}-to-{i+1}.txt")
    os.rename(deployment_txt_path, deployment_txt_path_new)
    deployment_paths.append(deployment_txt_path_new)
    print(f"Created transaction deployment file {deployment_txt_path_new}")
    print("Progress:", i+1, "/", len(program_paths))

# merge all deployment files into one named "all_deployments.txt"
all_deployments_path = os.path.join(program_deployments_path, "all_transfers.txt")
with open(all_deployments_path, "w") as f:
    for deployment_path in deployment_paths:
        f.write(open(deployment_path, "r").read())

print("Created all_transfers.txt")
